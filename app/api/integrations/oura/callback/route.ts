...
import { exchangeCode, fetchPersonalInfo } from "@/lib/oura";
...
export async function GET(request: Request) {
  ...
  if (error) {
    return htmlResponse(closingPage(false, `Oura: ${error}`));
  }
  if (!code || !state) {
  ...
  const info = await fetchPersonalInfo(tokens.access_token).catch(() => null);
  ...
  const { error: upErr } = await admin.from("user_integrations").upsert(
    {
      ...
      scope: tokens.scope ?? "personal daily workout",
...
function closingPage(ok: boolean, message?: string): string {
  const deepLink = ok
    ? "se7a://oura-connected"
    : `se7a://oura-connected?error=${encodeURIComponent(message ?? "unknown")}`;
  const title = ok ? "Oura connected." : "Oura connection failed.";
  const detail = ok
